...
def main():
    ...
    test_file = 'MFCC/test_converted.csv'
    ...
    rounded_labels = np.clip(np.abs(np.round(predicted_labels)), 0, 1)
    print_predictions(rounded_labels,"key_dev.txt","dev.results.MFCCONLY.txt")

    predicted_tests = model.predict(test_data)

    rounded_tests = np.clip(np.abs(np.round(predicted_tests)), 0, 1)
    print_predictions(rounded_tests,"key_test.txt","tests.results.MFCCONLY.txt")
